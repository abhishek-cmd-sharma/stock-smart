...
import { useProducts, useAddProduct, useUpdateProduct, useDeleteProduct, useInventoryStats, useUploadProductImage } from "@/hooks/useData";
...
export default function Inventory() {
  ...
  function openAdd() {
    setEditId(null);
    setForm({ name: "", category: "", price: "", quantity: "", expiry_date: "", batch_details: "", brand: "", barcode: "", image_url: "" });
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (productImageFile) {
        try {
          uploadedImageUrl = await uploadProductImage.mutateAsync(productImageFile);
        } catch (uploadErr: any) {
      ...
      >
                  ...
                  <div className="flex flex-col items-center gap-3 mb-4">
                    <div className="relative h-24 w-24 rounded-xl border-2 border-dashed border-border overflow-hidden bg-muted/30 flex items-center justify-center">
                      {(productImageFile || form.image_url) ? (
                        <img 
                          src={productImageFile ? URL.createObjectURL(productImageFile) : form.image_url} 
                          alt="Product preview" 
                    ...
                    <div>
                      <Label htmlFor="product-image" className="cursor-pointer">
                        <div className="bg-secondary text-secondary-foreground hover:bg-secondary/80 px-3 py-1.5 rounded-md text-xs font-medium transition-colors">
                          {form.image_url || productImageFile ? "Change Image" : "Upload Image"}
                        </div>
                      </Label>
                      <input 
                        type="file" 
          ...
          return (
            <div className="bg-card border border-border rounded-xl shadow-soft dark:shadow-soft-dark overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {allBatches.map((batch: any, idx: number) => {
                    ...
                    return (
                      <TableRow key={batch.id} className="hover:bg-muted/50 transition-colors">
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 overflow-hidden">
                              {batch.image_url ? (
                                <img src={batch.image_url} alt={batch.name} className="h-full w-full object-cover" />
                              ) : (
                            ...
                            <div>
                              ...
                              {hasBatchActiveDiscount && (
                                <Badge className="bg-amber-500 text-white text-[10px] mt-1">DISCOUNT</Badge>
                              )}
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge variant="outline" className="text-xs bg-background text-muted-foreground">{batch.category}</Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          ...
                          {hasBatchActiveDiscount && (
                            <div className="text-sm font-bold text-success">₹{batchFinalPrice.toLocaleString('en-IN')}</div>
                          )}
                        </TableCell>
                        <TableCell>
                          <span className={cn("px-2 py-1 rounded-md text-sm font-medium",
                            batch.quantity === 0 ? "bg-destructive/10 text-destructive" :
                            batch.quantity <= 10 ? "bg-warning/10 text-warning" :
                            "text-muted-foreground"
                          )}>
                            {batch.quantity}
                          </span>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={cn("text-xs font-medium",
                            bStatus === "expired" ? "bg-destructive/10 text-destructive border-destructive/20" :
                            bStatus === "critical" ? "bg-rose-500/10 text-rose-500 border-rose-500/20" :
                            bStatus === "warning" ? "bg-warning/10 text-warning border-warning/20" :
                          ...
                          <div className="flex items-center justify-end gap-1">
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => openEdit(batch)}>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-warning hover:text-warning" onClick={() => openDiscountDialog(batch.id, batch)}>
                            ...
                            {hasBatchActiveDiscount && (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => removeDiscount(batch.id)}>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => handleDelete(batch.id)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
          ...
          <div className="bg-card border border-border rounded-xl shadow-soft dark:shadow-soft-dark p-5 flex flex-col justify-between border-b-4 border-b-warning">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-muted-foreground">Active Discounts</span>
          ...
          <div className="bg-card border border-border rounded-xl shadow-soft dark:shadow-soft-dark p-5 flex flex-col justify-between border-b-4 border-b-destructive">
            <div className="flex items-center justify-between mb-2">
