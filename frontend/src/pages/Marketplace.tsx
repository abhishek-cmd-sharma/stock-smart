          ...
          {filtered.map(product => {
            ...
            return (
              <Card key={product.name} className="flex flex-col overflow-hidden hover:shadow-md transition-shadow">
                <CardHeader className="p-0 border-b relative group h-48 overflow-hidden bg-muted/20">
                  {product.image_url ? (
                    <img 
                  ...
                  {hasDiscount && (
                    <Badge variant="destructive" className="absolute top-4 right-4 bg-rose-500 shadow-sm flex items-center gap-1 z-10">
                      <Percent className="h-3 w-3" />
                      Save {discountPercent}%
                    </Badge>
                  )}
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
                    <Badge variant="secondary" className="text-[10px] w-fit font-semibold uppercase tracking-wider mb-1 bg-white/20 text-white border-none hover:bg-white/30 backdrop-blur-sm">
                      {product.category}
                    </Badge>
                    <CardTitle className="text-xl text-white font-bold truncate drop-shadow-sm">{product.name}</CardTitle>
                  </div>
                </CardHeader>
