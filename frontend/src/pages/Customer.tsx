...
export default function Customer() {
  ...
  const [mapProvider, setMapProvider] = useState<"leaflet" | "google">(
    getGoogleMapsApiKey() ? "google" : "leaflet",
  );
  ...
  const areaListings = pincode
    ? listings.filter(
        (l: any) => l.shop_pincode === pincode || extractPincode(l.shop_address) === pincode,
      )
    : [];
      ...
      <div className="page-header mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">
            ...
            {showAllProducts && (
              <span className="text-amber-500 ml-1"> - Set up profile for local results</span>
            )}
            .
          </p>
        </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((l: any) => {
                ...
                return (
                  ...
                  >
                    {/* Product Image Area */}
                    <div className="h-32 bg-muted/30 border-b border-border flex items-center justify-center relative overflow-hidden">
                      {l.image_url ? (
                        <img 
                          src={l.image_url} 
                          alt={l.name} 
                      ...
                      {hasDiscount && (
                        <Badge className="absolute top-2 right-2 bg-red-500 text-white gap-1 z-10">
                          <Percent className="h-3 w-3" />
                          {l.discount_type === "percentage"
                            ? l.discount_value
                            : Math.round((discount / Number(l.price)) * 100)}
                          % OFF
                        </Badge>
                      )}
                      {l.quantity === 0 && (
          ...
          {showMap && !searchedShops.some((s: any) => s.latitude && s.longitude) && (
            <div className="bg-muted/30 rounded-lg p-8 mb-6 text-center">
              ...
              <p className="text-muted-foreground">
                None of the shops currently have coordinates on the map.
              </p>
            </div>
          )}
          ...
          {selectedProduct && (
            <>
              ...
              <div className="space-y-6">
                {/* Product Image */}
                <div className="h-48 bg-muted/30 border border-border rounded-lg flex items-center justify-center relative overflow-hidden">
                  ...
                  {isDiscountActive(selectedProduct) && (
                    <Badge className="absolute top-3 right-3 bg-red-500 text-white text-lg px-3 py-1 z-10">
                    ...
                    <>
                      <span className="text-3xl font-bold text-primary">
                        ...
                        {Number(selectedProduct.price) -
                          calculateDiscount(Number(selectedProduct.price), selectedProduct)}
                      </span>
